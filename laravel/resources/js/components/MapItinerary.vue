...
const displayWaypoints = (newWaypoints) => {
  ...
  for (let i = 0; i < waypoints.value.length; i++) {
    L.marker(waypoints.value[i]).addTo(mapInstance.value).on('click', function(e) {
...
onMounted(() => {
  ...
  mapInstance.value = L.map('map').setView(storeMap.setViewVaud, storeMap.ZoomDefault);
  ...
  if(route.params.stepId) {
    initialMarkers.value = itineraries.value.map(itinerary => itinerary[0]);
    displayWaypoints({ value: itineraries.value[0].map(coords => L.latLng(...coords)) });    
  } else {
    initialMarkers.value = itineraries.value.map(itinerary => itinerary[0]);
    displayWaypoints({ value: itineraries.value[0].map(coords => L.latLng(...coords)) });
  }
...
defineExpose({ mapInstance, initialMarkers, modalIsVisible, toggleModalVisibility });
</script>