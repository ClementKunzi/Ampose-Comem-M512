...
console.log("Contenu initial du store:", selectedCategoryIds.value);
...
watch(
    ...
    (newVal) => {
        ...
        console.log(
            "Contenu du store après modification:",
...
const handleCategoryClick = (categoryId, selectAllCondition = false) => {
    if (selectAllCondition) {
        selectedCategory.value = storeCategories.categories.map(
            (cat) => cat.id
        );
    } else {
        if (categoryId === null) {
    ...
    setSelectedCategoryIds(
        selectedCategory.value.length > 0 ? selectedCategory.value : null
    );
    console.log(
        "Contenu du store après mise à jour:",
        selectedCategoryIds.value
    );
};
</script>
...
<template>
    <div class="sticky z-50 top-4 mb-9">
            ...
            >
                <div class="max-w-12 h-full">
                    <img
                        ...
                        :src="currentUser.getUserImageProfile()"
                        alt=""
                    />
                </div>
            </router-link>
        ...
        <div>
            <ul
                v-if="!isDesktop.value"
                class="overflow-scroll w-[calc(100%+2rem)] pl-4 ml-[-1rem] pb-4 mb-[-1rem] flex gap-1.5"
            >
                <li class="shrink-0">
                    <button
                ...
                >
                    ...
                    >
                        <img
                            :src="`storage/icons/${category.taxonomy.icon}`"
                            alt=""
                            class="w-4 h-4"
                        />
...
.active {
    background-color: rgb(
        117 64 67
    ); /* Exemple de couleur de fond pour l'élément actif */
